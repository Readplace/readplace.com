 ...
 * path from this script's output location. Lives in `platform` because three
 * project stacks (save-link, inbox, hutch) run the crawl fallback chain and
 * each needs the binary; the layer is published once here and read via the
 * platform StackReference.
 *
