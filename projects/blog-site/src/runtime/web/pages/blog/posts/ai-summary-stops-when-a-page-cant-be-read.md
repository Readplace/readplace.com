...
When you save a link, two jobs run behind it. One crawls the page, fetches it and cleans it into the reader copy you keep. The other writes the [TL;DR](/blog/how-ai-tldr-actually-works?utm_source=blog-ai-summary-stops-when-a-page-cant-be-read&utm_medium=internal&utm_content=post-how-ai-tldr-actually-works) from that copy. Each keeps its own state, and the summary waits on the crawl to hand it something to read.
...
The summary job did not read that verdict. It ran anyway, reached for the article text, and found none. An assertion in its path said the text must be there, so a missing body threw an error. The record went back onto the queue, ran again, and after a set number of tries landed in the [dead-letter queue](/view/docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-dead-letter-queues.html?utm_source=blog-ai-summary-stops-when-a-page-cant-be-read&utm_medium=internal&utm_content=read-docs-aws-amazon-com), where the summary was marked failed.
...
Save a link and the crawl decides what it is. A page it can read gets a TL;DR built from the copy you keep, [rebuilt later if a cleaner copy of the page arrives](/blog/ai-summary-updates-with-the-article?utm_source=blog-ai-summary-stops-when-a-page-cant-be-read&utm_medium=internal&utm_content=post-ai-summary-updates-with-the-article). A page it can't read gets a short skipped mark, and the summary step sets it down instead of circling it.

You still see that the page couldn't be read when you open it. The change is underneath, where a job that used to run in a loop now runs once and stops. Give Readplace a page it can read and one it can't, through [the browser extension](https://readplace.com/install) or [readplace.com](/?utm_source=blog-ai-summary-stops-when-a-page-cant-be-read&utm_medium=internal&utm_content=home), and watch the summary either land or step aside, with nothing left circling between the two.
