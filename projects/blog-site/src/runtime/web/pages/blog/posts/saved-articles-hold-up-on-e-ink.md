...
description: "11 pairs of ink and background across the Readplace reader and queue sat under their WCAG contrast floor, with article links rendering fainter than the prose around them. All 11 now clear it, thumbnails and the logout menu work with scripting off, and 3 test suites measure both pages the way an e-ink panel sees them."
...
The 2 filled amber buttons were in worse shape. Mark as read in the reader measured 3.62:1 in light and 2.95:1 in dark, under even the 3:1 line WCAG grants large text, and the call to action on a failed crawl repeated the same numbers. Both lightened on hover with a brightness filter, a move the brand guidelines ban outright. A solid amber fill leaves no way out here: white ink needs a darker fill under it, and nothing darker existed for the hover to become. Both buttons took the outlined look the queue's own Mark as read already wore, where rest and hover each clear 5.06:1.
...
Two findings had nothing to do with contrast. The queue's thumbnails were revealed by an inline `onload` handler on a hidden wrapper, and a browser with scripting off runs no handlers, so each card kept a blank 120 by 78 strip where its image belonged. The handler and the hidden state were one mechanism, and both are gone.
...
The next time the nearest screen is an e-reader's built-in browser, [your queue](/) reads there the way it reads on this one, and the clean copy of each save is waiting behind its card.
