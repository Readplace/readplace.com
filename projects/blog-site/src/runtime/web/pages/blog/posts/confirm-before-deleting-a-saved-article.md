...
The confirmation is a [native HTML popover](/view/developer.mozilla.org/en-US/docs/Web/API/Popover_API). The delete button points at the panel by id, the browser opens it, and the parts that make a dialog behave like a dialog come with it. Esc closes it. A click outside it closes it. The rest of the page dims behind a backdrop the browser paints. None of that is code I keep working.

> **The popover attribute hands the whole dialog to the browser: the scrim, the Esc key, the click-outside, all of it.**

That saves more than it looks like it does. A confirmation dialog is the kind of small thing people reach for a library to build, and the library brings a bundle to download, focus handling to get right, and a scrim to style and to trap clicks. The popover attribute hands all of it to the browser. There is no client script for the panel, no route it posts to on the way open, and no query parameter tracking whether it's showing. The whole thing is an attribute on a button and a panel it names.
