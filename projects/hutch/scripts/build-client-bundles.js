...
const BUNDLES = [
	...
	},
	{
		entry: path.join(
			PROJECT_ROOT,
			"src/runtime/web/shared/highlights/highlights.client.ts",
		),
		...
		footer: [
			...
			"  },",
			"  promptNote: function () { return window.prompt('Add a note (optional):', ''); },",
			"  postHighlight: function (input) {",
			"    var panel = window.document.querySelector('[data-highlights]');",
			"    if (!panel) return Promise.resolve(null);",
			"    var url = panel.getAttribute('data-highlights-create-url');",
