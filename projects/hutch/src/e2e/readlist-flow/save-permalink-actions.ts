	...
	return (authProgress) => ({
		...
		'delete-permalink-article': {
			...
			execute: async (page) => {
				...
				// The delete propagates to the eventually-consistent readlist GSI
				// asynchronously, so the first post-redirect render can still list the
				// article. Reload a fresh read until the empty-readlist marker appears
				// rather than asserting once against a possibly-stale render.
				await expect(async () => {
					await page.reload()
					await expect(page.locator('[data-test-empty-readlist]')).toBeVisible({ timeout: 2000 })
