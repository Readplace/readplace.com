...
import type { ReadlistActionKey, SaveArticleKey, PaginationArticleKey } from './action-catalog'
...
export function createLocalTestArticles(baseUrl: string): TestArticleData {
	return {
		...
		urls: [
			`${baseUrl}/e2e/article/readlist-flow-1?title=${encodeURIComponent("Readplace E2E Readlist Article One")}`,
			`${baseUrl}/e2e/article/readlist-flow-2?title=${encodeURIComponent("Readplace E2E Readlist Article Two")}`,
			`${baseUrl}/e2e/article/readlist-flow-3?title=${encodeURIComponent("Readplace E2E Readlist Article Three")}`,
			`${baseUrl}/e2e/article/readlist-flow-4?title=${encodeURIComponent("Readplace E2E Readlist Article Four")}`,
		],
		titles: [
			"Readplace E2E Readlist Article One",
			"Readplace E2E Readlist Article Two",
			"Readplace E2E Readlist Article Three",
			"Readplace E2E Readlist Article Four",
		],
...
export function createReadlistActions(
	authProgress: AuthProgress,
	progress: ReadlistProgress,
	testData: TestArticleData,
): Record<ReadlistActionKey, PageAction> {
	assert.equal(
	...
	return {
		...
		'resave-existing-article-triggers-refresh': {
			isAvailable: async (page) => {
				...
				if (!(await isOnPage(page, 'page-readlist'))) return false
				const saveForm = page.locator('[data-test-form="save-article"]')
				return saveForm.isVisible().catch(() => false)
			},
			execute: async (page) => {
