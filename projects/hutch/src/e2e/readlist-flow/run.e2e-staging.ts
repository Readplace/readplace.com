...
import type { ReadlistProgress, TestArticleData } from './readlist-actions'
...
test.describe('Readlist management flow (staging)', () => {
	test('signup, logout, login, add articles, pagination, sort, read, delete, verify tabs', async ({ page, baseURL }) => {
		...
		const stagingArticles: TestArticleData = {
			urls: [
				fixtureUrl('readlist-1'),
				fixtureUrl('readlist-2'),
				fixtureUrl('readlist-3'),
				fixtureUrl('readlist-4'),
			],
