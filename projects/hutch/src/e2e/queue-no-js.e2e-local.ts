...
test.describe("The queue is whole without client JavaScript", () => {
	test.use({
		timezoneId: "UTC",
	...
	async function seedArticleWithThumbnail(page: Page, stamp: string): Promise<string> {
		const email = `queue-no-js-${stamp}@example.com`;
	...
	test("a card thumbnail is visible with scripting off", async ({ page }, testInfo) => {
		...
		const thumbnail = page.locator(".queue-article__thumbnail-link").first();
