...
const GENERATED_EXCERPT =
	"Attention is the scarce resource, and the readlist is where it gets spent: every card you skim is a decision about what you will never read, which is why the teaser has to carry the whole thought instead of trailing off into an ellipsis that costs a click.";
...
async function seedCard(
	...
	const response = await page.request.post(`${BASE_URL}/e2e/seed-crawled-article`, {
		data: {
			url: params.url,
			title: params.title,
			content: "<p>Seeded body for the readlist-card excerpt measurement.</p>",
...
test.describe("Readlist card excerpt", () => {
	...
	test("seats a generated excerpt whole and clamps an unbounded crawler-parsed one", async ({
		...
		const email = `readlist-excerpt-${stamp}@example.com`;
