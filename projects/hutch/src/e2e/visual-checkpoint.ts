...
export async function captureCheckpoint(page: Page, checkpoint: VisualCheckpoint): Promise<void> {
	await checkpoint.settled(page);
	const scrollLeftByAction = await page.evaluate(() => ({ x: window.scrollX, y: window.scrollY }));
