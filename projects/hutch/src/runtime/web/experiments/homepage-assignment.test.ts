...
describe("writeHomepageAssignment", () => {
	it("writes campaign:epoch:slug as an httpOnly cookie the signup path can read back", () => {
		...
		expect(cookies[0].value).toBe("homepage-split:3:variant-b");
...
describe("readHomepageAssignment", () => {
	it("resolves a current campaign+epoch cookie to its variant", () => {
		assert.equal(
			readHomepageAssignment(reqWithCookie("homepage-split:3:variant-a"), HOMEPAGE_SPLIT),
			VARIANT_A,
		);
		assert.equal(
			readHomepageAssignment(reqWithCookie("homepage-split:3:variant-b"), HOMEPAGE_SPLIT),
			VARIANT_B,
		);
	});
	...
	it("returns undefined when the epoch matches but the slug is unknown", () => {
		expect(
			readHomepageAssignment(reqWithCookie("homepage-split:3:variant-z"), HOMEPAGE_SPLIT),
		).toBeUndefined();
	});
});
...
describe("readHomepageVariantSlug", () => {
	it("returns the arm slug against the live config", () => {
		expect(readHomepageVariantSlug(reqWithCookie("homepage-split:3:variant-b"))).toBe("variant-b");
	});
