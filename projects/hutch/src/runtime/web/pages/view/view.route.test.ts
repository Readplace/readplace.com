...
describe("View routes", () => {
	...
	describe("GET /view trailing-slash content fallback", () => {
		function eventsFor(
			applyParseResult: ReturnType<typeof createFakeApplyParseResult>,
		...
		it("302-redirects a slash-less url with no content of its own to its '/' variant when only the '/' one has content (e.g. extension-saved)", async () => {
			const parseArticle: ParseArticle = async () => buildParseResult();
			const fixture = createDefaultTestAppFixture(TEST_APP_ORIGIN);
			const applyParseResult = createFakeApplyParseResult({ articleStore: fixture.articleStore, articleCrawl: fixture.articleCrawl, parseArticle });
			const harness = useApp({ ...fixture, parser: { parseArticle, crawlArticle: fixture.parser.crawlArticle }, events: eventsFor(applyParseResult, fixture) });
			// Seed ONLY the "/" variant with content, as the extension would (the
			// browser lands on the "/" form after the origin's 308).
			await fixture.articleStore.saveArticleGlobally({
				url: `${ARTICLE_URL}/`,
				metadata: { title: "Slash", siteName: "example.com", excerpt: "", wordCount: 100 },
				estimatedReadTime: calculateReadTime(100),
				savedAt: new Date("2026-06-01T00:00:00.000Z"),
			});
			await fixture.articleStore.writeContent({ url: `${ARTICLE_URL}/`, content: "<article><p>Slash body copy.</p></article>" });
		...
		it("does not redirect when neither the slash-less url nor its '/' variant has content (falls through to the normal first-visit crawl)", async () => {
			const parseArticle: ParseArticle = async () => buildParseResult();
			const fixture = createDefaultTestAppFixture(TEST_APP_ORIGIN);
			const applyParseResult = createFakeApplyParseResult({ articleStore: fixture.articleStore, articleCrawl: fixture.articleCrawl, parseArticle });
			const harness = useApp({ ...fixture, parser: { parseArticle, crawlArticle: fixture.parser.crawlArticle }, events: eventsFor(applyParseResult, fixture) });
			...
			expect(response.status).toBe(200);
			expect(new JSDOM(response.text).window.document.querySelector("[data-test-reader-title]")?.textContent).toBe("Hello World");
		});

		it("renders a trailing-slash url in place without attempting the fallback (never loops)", async () => {
			const parseArticle: ParseArticle = async () => buildParseResult();
			const fixture = createDefaultTestAppFixture(TEST_APP_ORIGIN);
			const applyParseResult = createFakeApplyParseResult({ articleStore: fixture.articleStore, articleCrawl: fixture.articleCrawl, parseArticle });
			const harness = useApp({ ...fixture, parser: { parseArticle, crawlArticle: fixture.parser.crawlArticle }, events: eventsFor(applyParseResult, fixture) });
