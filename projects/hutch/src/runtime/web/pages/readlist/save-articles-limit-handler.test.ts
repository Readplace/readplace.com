...
describe("initSaveArticlesLimitHandler", () => {
	it("intercepts entity.too.large errors at the configured limit and emits a Siren 422", () => {
		const handler = initSaveArticlesLimitHandler({
			logError: () => {},
			maxBytes: 1024,
		});
		...
		expect(calls.body).toEqual(expect.objectContaining({
			class: ["error"],
			properties: expect.objectContaining({ code: "save-articles-too-large" }),
		}));
	...
	it("logs the size violation through the injected logError so it lands on the alarm path", () => {
		...
		expect(logged[0]?.message).toContain("save-articles request body exceeded");
