...
const REDIRECT_TO_READLIST: ReaderPermalinkResult = {
	kind: "redirect",
...
export function initReaderPermalink(deps: ReaderPermalinkDeps) {
	return async function resolveReaderPermalink(
		...
		if (!parsedId.success) return REDIRECT_TO_READLIST;
		...
		if (input.requesterId === undefined && wantsOwnerLogin(input.query)) {
			return {
				...
				redirect: {
					statusCode: 303,
					location: `/login?return=${encodeURIComponent(ownerReaderLoginPath(parsedId.data, input.query))}`,
				},
			};
		}
