...
const MAIN_SELECTOR = "main[data-test-readlist-page]";
...
describe("`/queue` behaviour", () => {
	...
	it("walks a multi-page listing forwards and back through the pagination controls", async () => {
		...
		const firstPage = parse((await agent.get("/queue")).text);
	...
	it("swaps its counts fragment onto ids the page it came from actually rendered", async () => {
		...
		const page = parse((await agent.get("/queue")).text);
	...
	it("re-renders its own listing when reading the last row would leave the page adrift", async () => {
		...
		const response = await agent
			.post(`/queue/${articleId}/status?swap=card`)
			.set("HX-Request", "true")
			.type("form")
			.send({ status: "read" });
		...
		assert(
			doc.querySelector(MAIN_SELECTOR),
			"the fallback re-renders the whole readlist page",
		);
	...
	it("keeps the reader's filters on the card's next poll, and stops polling at the budget", async () => {
		...
		const polling = parse(
			(await agent.get(`/queue/${articleId}/card?poll=1&tab=done&order=asc`)).text,
		...
		expect(nextUrl.searchParams.get("feature")).toBeNull();

		const exhausted = parse(
			(await agent.get(`/queue/${articleId}/card?poll=${MAX_POLLS}`)).text,
		).querySelector("[data-test-article]");
	...
	it("arms the save skeleton above the row a save will land in, and only where a save can land", async () => {
		...
		const fresh = parse((await agent.get("/queue")).text);
		...
		const populated = parse((await agent.get("/queue")).text);
		...
		const doneTab = parse((await agent.get("/queue?tab=done")).text);
