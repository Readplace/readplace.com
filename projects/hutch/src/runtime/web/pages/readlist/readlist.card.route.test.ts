...
describe("Readlist routes", () => {
	describe("Thumbnail", () => {
		it("should render thumbnail when article has og:image", async () => {
			...
			const thumbnail = doc.querySelector(".readlist-article__thumbnail");
		...
		it("should link thumbnail to reader view when content exists", async () => {
			...
			const response = await agent.get("/queue");
			const doc = new JSDOM(response.text).window.document;
			const thumbnailLink = doc.querySelector(".readlist-article__thumbnail")?.closest("a");
		...
		it("should not render thumbnail when page has no images", async () => {
			...
			expect(card.querySelectorAll(".readlist-article__thumbnail-link").length).toBe(0);
		});
		...
		it("should render the thumbnail with no script hook to reveal it", async () => {
			...
			expect(link.getAttribute("class")).toBe("readlist-article__thumbnail-link");
		...
		it("should remove thumbnail wrapper when image error event fires", async () => {
			...
			const thumbnail = dom.window.document.querySelector<HTMLImageElement>(".readlist-article__thumbnail");
			const link = thumbnail?.closest("a");
			assert(link, "thumbnail link must be rendered before the error event");
			expect(link.classList.contains("readlist-article__thumbnail-link")).toBe(true);

			thumbnail?.dispatchEvent(new dom.window.Event("error"));

			const card = dom.window.document.querySelector(".readlist-article");
