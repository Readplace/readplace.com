...
describe("Readlist onboarding — Next Read milestone", () => {
	...
	it("still renders the readlist when stamping the milestone throws", async () => {
		const fixture = createDefaultTestAppFixture(TEST_APP_ORIGIN);
	...
	it("makes one onboarding read, and never the save count, on a device with no installable client", async () => {
		...
		// One read: the same row carries the mark-read-across-readlists acknowledgement,
