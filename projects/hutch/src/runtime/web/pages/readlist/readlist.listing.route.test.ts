...
describe("Readlist routes", () => {
	...
	describe("GET /queue (authenticated)", () => {
		it("should render the empty readlist", async () => {
			const harness = useApp(createDefaultTestAppFixture(TEST_APP_ORIGIN));
			const { auth } = harness;
	...
	describe("GET /queue — degraded batched summary/crawl reads", () => {
		it("still renders the cards (minus chips) and logs when the batch reads reject", async () => {
			...
			expect(response.status).toBe(200);
			const doc = new JSDOM(response.text).window.document;
			expect(doc.querySelectorAll("[data-test-article-list] .readlist-article").length).toBe(1);
	...
	describe("Listing content projection", () => {
		it("asks the store to skip the article body when listing the readlist", async () => {
			const fixture = createDefaultTestAppFixture(TEST_APP_ORIGIN);
	...
	describe("Filter and sort", () => {
		it("should filter by status", async () => {
			...
			expect(unreadDoc.querySelectorAll(".readlist-article").length).toBe(2);

			const readResponse = await agent.get("/queue?status=read");
			const readDoc = new JSDOM(readResponse.text).window.document;
			expect(readDoc.querySelectorAll(".readlist-article").length).toBe(0);
		});
		...
		it("should order Done tab by readAt descending (most recently read first)", async () => {
			...
			const cards = readlistDoc.querySelectorAll("[data-test-article-list] .readlist-article");
			...
			const readIds = Array.from(
				readDoc.querySelectorAll("[data-test-article-list] .readlist-article"),
		...
		it("should order Done tab by readAt ascending when order=asc", async () => {
			...
			const readlistDoc = new JSDOM(readlistResponse.text).window.document;
			const idByUrl = new Map<string, string>();
			for (const card of readlistDoc.querySelectorAll("[data-test-article-list] .readlist-article")) {
				const url = card.querySelector("[data-test-article-url]")?.getAttribute("href");
				const id = card.getAttribute("data-test-article");
				assert(url && id, "article card must expose url and id");
				idByUrl.set(url, id);
	...
	describe("Re-saving a read article marks it unread", () => {
		it("should mark a read article as unread when saved again via form", async () => {
			...
			expect(afterReadDoc.querySelectorAll(".readlist-article").length).toBe(0);
		});
	});
