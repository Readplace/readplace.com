...
async function readerWithTwoReadlists(harness: TestHarness): Promise<{
	agent: TestAgent;
	readlist: string;
	articleId: string;
}> {
	...
	return { agent, readlist, articleId };
}
...
describe("Mark-as-read confirmation", () => {
	it("stays out of the way of a reader who owns no custom readlist", async () => {
	...
	it("renders one panel per article at <main> level once the reader owns a readlist", async () => {
		const harness = useApp(createDefaultTestAppFixture(TEST_APP_ORIGIN));
	...
	it("names only the readlists that article actually sits in, one bullet each", async () => {
		const harness = useApp(createDefaultTestAppFixture(TEST_APP_ORIGIN));
		const { agent } = await readerWithTwoReadlists(harness);
	...
	it("marks the article read in every readlist when the reader confirms once", async () => {
		const harness = useApp(createDefaultTestAppFixture(TEST_APP_ORIGIN));
