...
export interface ReadlistTab extends TabDefinition {
	id: TabId;
}
...
export const READLIST_TABS: readonly ReadlistTab[] = TAB_IDS.map((id) => ({
	id,
	...TAB_DEFINITIONS[id],
}));

export const READLIST_TAB_STATUSES: readonly { label: string; status: ArticleStatus }[] =
	READLIST_TABS.map((tab) => ({ label: tab.label, status: tab.query.status }));
