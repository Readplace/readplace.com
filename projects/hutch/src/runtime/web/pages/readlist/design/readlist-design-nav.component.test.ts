...
function renderNav(overrides: Partial<Parameters<typeof buildReadlistDesignNav>[0]> = {}): Document {
	const input = {
		readlists: READLISTS,
	...
	return new JSDOM(`<main>${renderReadlistDesignNav(buildReadlistDesignNav(input))}</main>`).window.document;
}
...
describe("buildReadlistDesignNav", () => {
	...
	it("opens a custom readlist's menu on an Edit control that targets its rename popover", () => {
		...
		expect(edit.getAttribute("popovertarget")).toBe(readlistDesignRenamePopoverId(WORK.slug));
	...
	it("backs the delete trigger with a plain-post fallback carrying the design flag and the return state", () => {
		const doc = renderNav({ activeSlug: WORK.slug });
	...
	it("points each readlist's link at its own listing, carrying the design flag and its own tracking token", () => {
		...
		expect(forWork.params.get("feature")).toBe("design");
	...
	it("marks the viewed readlist's link and item as the selected one, and leaves the others plain", () => {
		...
		expect(readlistLink(doc, "work").getAttribute("class")).toBe(
			"readlist-design-nav__link readlist-design-nav__link--active",
		);
		expect(readlistLink(doc, "work").parentElement?.className).toBe(
			"readlist-design-nav__item readlist-design-nav__item--active",
		);
		expect(readlistLink(doc, "default").getAttribute("class")).toBe("readlist-design-nav__link");
		expect(readlistLink(doc, "default").parentElement?.className).toBe("readlist-design-nav__item");
	});
	...
	it("starts a new readlist by posting, tagged for funnel attribution", () => {
		...
		const form = doc.querySelector("form.readlist-design-nav__new-form");
