import { readFileSync } from "node:fs";
import { join } from "node:path";
import type { IconName } from "@packages/ui-icons";
import { CONFIRM_POPOVER_STYLES, render, withInternalTracking } from "@packages/web-shell";
import type { CspNonce, PageBody } from "@packages/web-shell";
import type { DeviceClass } from "@packages/web-analytics";

import { NAV_HIDE_SCRIPT } from "../../../shared/reader-nav-script";
import { renderIllustration } from "../../../shared/illustrations/illustrations";
import {
	ONBOARDING_DESIGN_STYLES,
	OnboardingDesignChecklist,
} from "../../../onboarding/onboarding-design.component";
import type { OnboardingContext } from "../../../onboarding/onboarding.types";
import { SAVE_TIP_SCRIPT, type SaveTip } from "../../../shared/save-tip/save-tip.component";
import type { SaveTipState } from "../../../shared/save-tip/save-tip";
import {
	SUBSCRIBE_PLANS_STYLES,
	renderSubscribePlansPopover,
} from "../../../shared/subscribe-plans/subscribe-plans.component";
import { renderMarkStatusConfirm } from "../mark-status-confirm.component";
import { renderDeleteConfirm } from "../readlist-card/delete-confirm.component";
import {
	readlistDeleteConfirmPopoverId,
	renderReadlistDeleteConfirm,
} from "../readlist-delete-confirm.component";
import { formatUnreadLabel } from "../readlist-filters.component";
import { renderReadlistCountsTrigger, renderStatusToast } from "../readlist-mutation-fragments";
import {
	renderReadlistSaveSkeleton,
	toReadlistSaveSkeletonDisplayModel,
} from "../readlist-save-skeleton.component";
import { READER_PAGE_SCRIPTS, renderReaderSkeleton } from "../reader-skeleton/reader-skeleton.component";
import type { ReadlistRailViewModel } from "../readlist.component";
import { DEFAULT_READLIST } from "../readlist.nav";
import { READLIST_STYLES } from "../readlist.styles";
import { READLIST_TABS, type TabId, tabQuery } from "../readlist.tabs";
import { READLIST_SAVE_PATH, buildReadlistUrl, readlistDeletePath } from "../readlist.url";
import type { ReadlistArticleViewModel, ReadlistViewModel } from "../readlist.viewmodel";
import { readlistDesignAlertFor } from "./readlist-design-alerts";
import { renderReadlistDesignCard, toReadlistDesignCardDisplayModel } from "./readlist-design-card.component";
import { savedArticlesLabel, showingLabel } from "./readlist-design-counts.component";
import {
	designFeatureParams,
	readlistDesignReturnQuery,
	withDesignFeature,
} from "./readlist-design-feature";
import { buildReadlistDesignNav, renderReadlistDesignNav } from "./readlist-design-nav.component";
import {
	READLIST_DESIGN_RENAME_SCRIPT,
	renderReadlistDesignRename,
} from "./readlist-design-rename.component";
import {
	renderReadlistDesignSubscription,
	toReadlistDesignSubscriptionDisplayModel,
} from "./readlist-design-subscription.component";
import { READLIST_DESIGN_STYLES } from "./readlist-design.styles";

const TEMPLATE = readFileSync(join(__dirname, "readlist-design.template.html"), "utf-8");

export const READLIST_DESIGN_BODY_CLASS = "page-readlist page-readlist-design";

interface ReadlistDesignOnboarding {
	context: OnboardingContext;
	dismissed: boolean;
	completedBefore: boolean;
	completionUnearned: boolean;
}

export interface ReadlistDesignPageOptions {
	cspNonce: CspNonce;
	deviceClass: DeviceClass;
	readlistHoldsArticles: boolean;
	knownUnreadCount?: number;
	rail: ReadlistRailViewModel;
	saveTip: SaveTip;
	saveUrl?: string;
	onboarding: ReadlistDesignOnboarding;
	query: Record<string, unknown>;
}

interface DesignTab {
	linkClass: string;
	href: string;
	label: string;
	testFilter: string;
	isActive: boolean;
	labelId?: string;
	widestLabel?: string;
}

interface EmptyAction {
	key: string;
	href: string;
	label: string;
}

interface EmptyState {
	title: string;
	text: string;
	actions: EmptyAction[];
}

const NOTHING_SAVED: Omit<EmptyState, "actions"> = {
	title: "Nothing saved yet",
	text: "Save your first article by pasting a link above, or use the Readplace browser extension to save it in one click.",
};

const CAUGHT_UP: Omit<EmptyState, "actions"> = {
	title: "You're all caught up",
	text: "There are no articles left in your To Read list. Save something new to keep your reading list going.",
};

const NOTHING_READ: Omit<EmptyState, "actions"> = {
	title: "No finished articles yet",
	text: "Once you finish an article and mark it as read, you'll find it here.",
};

const CUSTOM_READLIST_EMPTY: Omit<EmptyState, "actions"> = {
	title: "No articles in this readlist yet",
	text: `Every link you save lands in ${DEFAULT_READLIST.label}. Open an article there to add it to this readlist.`,
};

function emptyState(input: {
	tab: TabId;
	readlistHoldsArticles: boolean;
	isDefaultReadlist: boolean;
	unreadUrl: string;
	defaultReadlistUrl: string;
}): EmptyState {
	if (!input.readlistHoldsArticles) {
		return input.isDefaultReadlist
			? {
					...NOTHING_SAVED,
					actions: [
						{
							key: "install",
							href: withInternalTracking("/install", { source: "queue-empty", content: "install" }),
							label: "Install browser extension",
						},
					],
				}
			: {
					...CUSTOM_READLIST_EMPTY,
					actions: [
						{
							key: "open-default",
							href: input.defaultReadlistUrl,
							label: `Go to ${DEFAULT_READLIST.label}`,
						},
					],
				};
	}
	return input.tab === "queue"
		? { ...CAUGHT_UP, actions: [] }
		: {
				...NOTHING_READ,
				actions: [{ key: "view-unread", href: input.unreadUrl, label: "View Unread Articles" }],
			};
}

function designTabs(input: {
	vm: ReadlistViewModel;
	knownUnreadCount?: number;
}): DesignTab[] {
	const { readlist, tab: activeTab, order } = input.vm.filters;
	return READLIST_TABS.map((tab) => {
		const isActive = tab.id === activeTab;
		return {
			linkClass: `readlist-design-tabs__link${isActive ? " readlist-design-tabs__link--active" : ""}`,
			href: withInternalTracking(
				buildReadlistUrl({ readlist, tab: tab.id, order }, designFeatureParams(true)),
				{ source: "queue-filters", content: tab.trackingContent },
			),
			label:
				tab.labelId !== undefined && input.knownUnreadCount !== undefined
					? formatUnreadLabel(input.knownUnreadCount)
					: tab.label,
			testFilter: tab.testFilter,
			isActive,
			labelId: tab.labelId?.(readlist),
			widestLabel: tab.labelId === undefined ? undefined : formatUnreadLabel(Number.MAX_SAFE_INTEGER),
		};
	});
}

function withDesignConfirmLinks(article: ReadlistArticleViewModel): ReadlistArticleViewModel {
	return {
		...article,
		...(article.deleteConfirm === undefined
			? {}
			: { deleteConfirm: { ...article.deleteConfirm, url: withDesignFeature(article.deleteConfirm.url) } }),
		...(article.markStatusConfirm === undefined
			? {}
			: {
					markStatusConfirm: {
						...article.markStatusConfirm,
						url: withDesignFeature(article.markStatusConfirm.url),
					},
				}),
	};
}

function readlistPanels(rail: ReadlistRailViewModel): { deleteConfirms: string; renames: string } {
	if (!rail.canCreate) return { deleteConfirms: "", renames: "" };
	const owned = rail.readlists.filter((readlist) => readlist.slug !== DEFAULT_READLIST.slug);
	const returnQuery = readlistDesignReturnQuery({ readlist: rail.activeReadlist.slug });
	return {
		deleteConfirms: owned
			.map((readlist) =>
				renderReadlistDeleteConfirm({
					popoverId: readlistDeleteConfirmPopoverId(readlist.slug),
					url: `${readlistDeletePath(readlist.slug)}${returnQuery}`,
					label: readlist.label,
					destinations: owned.filter((other) => other.slug !== readlist.slug),
					illustrationHtml: renderIllustration("trash-can"),
				}),
			)
			.join("\n"),
		renames: owned
			.map((readlist) => renderReadlistDesignRename({ slug: readlist.slug, label: readlist.label }))
			.join("\n"),
	};
}

function countLabelBeforeCounts(vm: ReadlistViewModel): string {
	const totalIsKnown = vm.currentPage === 1 && !vm.paginationUrls.next;
	return totalIsKnown ? savedArticlesLabel(vm.articles.length) : "Saved Articles";
}

export function ReadlistDesignPage(vm: ReadlistViewModel, options: ReadlistDesignPageOptions): PageBody {
	const { filters } = vm;
	const isDefaultReadlist = filters.readlist === DEFAULT_READLIST.slug;
	const effectiveOrder = filters.order ?? tabQuery(filters.tab).defaultOrder;
	const nextOrder = effectiveOrder === "desc" ? "asc" : "desc";
	const sort: { label: string; iconName: IconName } =
		effectiveOrder === "desc"
			? { label: "Newest first", iconName: "arrow-down" }
			: { label: "Oldest first", iconName: "arrow-up" };
	const defaultReadlistUrl = buildReadlistUrl({}, designFeatureParams(true));
	const alert = readlistDesignAlertFor(options.query);
	const banner = vm.subscriptionBanner;
	const articles = vm.articles.map(withDesignConfirmLinks);
	const panels = readlistPanels(options.rail);
	const empty = emptyState({
		tab: filters.tab,
		readlistHoldsArticles: options.readlistHoldsArticles,
		isDefaultReadlist,
		unreadUrl: withInternalTracking(
			buildReadlistUrl({ readlist: filters.readlist, tab: "queue" }, designFeatureParams(true)),
			{ source: "queue-empty", content: "view-unread" },
		),
		defaultReadlistUrl: withInternalTracking(defaultReadlistUrl, {
			source: "queue-empty",
			content: "open-default",
		}),
	});
	const saveTipState: SaveTipState = options.saveTip.state;

	const content = render(TEMPLATE, {
		readlistNavHtml: renderReadlistDesignNav(
			buildReadlistDesignNav({
				readlists: options.rail.readlists,
				activeSlug: options.rail.activeReadlist.slug,
				newReadlistAction: withDesignFeature(options.rail.newReadlistAction),
				canCreate: options.rail.canCreate,
			}),
		),
		alertStateClass: alert ? "readlist-design__alert--visible" : "readlist-design__alert--hidden",
		alertTitle: alert?.title ?? "",
		alertBody: alert?.body ?? "",
		statusToastHtml: vm.statusFlash
			? renderStatusToast({ ...vm.statusFlash, undoUrl: withDesignFeature(vm.statusFlash.undoUrl) })
			: "",
		saveCardClass: isDefaultReadlist ? "readlist-design-save--visible" : "readlist-design-save--hidden",
		saveFormStateClass: vm.accessIsReadOnly
			? "readlist-design-save__form--disabled"
			: "readlist-design-save__form--enabled",
		saveInputStateClass: vm.saveErrorCode || vm.errors?.length
			? "readlist-design-save__input--invalid"
			: "readlist-design-save__input--valid",
		saveAction: withInternalTracking(
			`${READLIST_SAVE_PATH}${readlistDesignReturnQuery({ ...filters, readlist: DEFAULT_READLIST.slug })}`,
			{ source: "queue", content: "save" },
		),
		saveUrl: options.saveUrl,
		saveTipState,
		accessIsReadOnly: vm.accessIsReadOnly,
		saveError: vm.errors?.[0]?.message,
		saveErrorCode: vm.saveErrorCode,
		importFlash: vm.importFlash,
		hasImportSkipped: Boolean(vm.importSkipped && vm.importSkipped.entries.length > 0),
		importSkippedEntries: vm.importSkipped?.entries ?? [],
		importSkippedAndMore: vm.importSkipped?.andMore,
		tabs: designTabs({ vm, knownUnreadCount: options.knownUnreadCount }),
		countsSpanHtml: renderReadlistCountsTrigger({ countsUrl: withDesignFeature(vm.countsUrl) }),
		countLabel: countLabelBeforeCounts(vm),
		sortUrl: withInternalTracking(
			buildReadlistUrl(
				{ readlist: filters.readlist, tab: filters.tab, order: nextOrder },
				designFeatureParams(true),
			),
			{ source: "queue-sort", content: "sort", term: nextOrder },
		),
		sortLabel: sort.label,
		sortIconName: sort.iconName,
		saveSkeletonHtml: renderReadlistSaveSkeleton(
			toReadlistSaveSkeletonDisplayModel({ filters, accessIsReadOnly: vm.accessIsReadOnly }),
		),
		isEmpty: vm.isEmpty,
		emptyIllustrationHtml: renderIllustration("book-lightbulb"),
		emptyTitle: empty.title,
		emptyText: empty.text,
		emptyActions: empty.actions,
		hasArticles: !vm.isEmpty,
		articleHtmls: articles.map((article, index) =>
			renderReadlistDesignCard(
				toReadlistDesignCardDisplayModel(article, { isFirst: index === 0, deviceClass: options.deviceClass }),
			),
		),
		paginationStateClass: vm.isEmpty
			? "readlist-design-pagination--hidden"
			: "readlist-design-pagination--visible",
		showingLabel: showingLabel({ rowsOnPage: vm.articles.length }),
		prevUrl: vm.paginationUrls.prev
			? withInternalTracking(withDesignFeature(vm.paginationUrls.prev), {
					source: "queue-pagination",
					content: "prev",
				})
			: undefined,
		nextUrl: vm.paginationUrls.next
			? withInternalTracking(withDesignFeature(vm.paginationUrls.next), {
					source: "queue-pagination",
					content: "next",
				})
			: undefined,
		currentPage: vm.currentPage,
		subscriptionHtml: renderReadlistDesignSubscription(toReadlistDesignSubscriptionDisplayModel(banner)),
		onboardingHtml: OnboardingDesignChecklist(options.onboarding.context, {
			dismissed: options.onboarding.dismissed,
			completedBefore: options.onboarding.completedBefore,
			completionUnearned: options.onboarding.completionUnearned,
			returnQuery: readlistDesignReturnQuery(filters),
		}),
		readlistRenamesHtml: panels.renames,
		readlistDeleteConfirmHtml: panels.deleteConfirms,
		deleteConfirmsHtml: articles
			.flatMap((article) =>
				article.deleteConfirm === undefined
					? []
					: [
							renderDeleteConfirm({
								confirm: article.deleteConfirm,
								title: article.title,
								illustrationHtml: renderIllustration("trash-can"),
							}),
						],
			)
			.join("\n"),
		markStatusConfirmsHtml: articles
			.flatMap((article) =>
				article.markStatusConfirm === undefined
					? []
					: [
							renderMarkStatusConfirm({
								confirm: article.markStatusConfirm,
								source: "queue-card",
								lead: article.title,
							}),
						],
			)
			.join("\n"),
		subscribePlansHtml:
			banner.state === "trial-countdown" || banner.state === "inactive"
				? renderSubscribePlansPopover({ source: "queue-banner" })
				: "",
		saveTipHtml: options.saveTip.html,
		readerSkeletonHtml: renderReaderSkeleton({ cspNonce: options.cspNonce }),
	});

	const scripts = [NAV_HIDE_SCRIPT, SAVE_TIP_SCRIPT, READLIST_DESIGN_RENAME_SCRIPT, READER_PAGE_SCRIPTS];
	if (options.saveUrl) scripts.push(autoSubmitScript(options.cspNonce));

	return {
		seo: {
			title: `${options.rail.activeReadlist.label} — Readplace`,
			description: "Your saved articles readlist.",
			canonicalUrl: "/queue",
			robots: "noindex, nofollow",
		},
		styles: `${READLIST_STYLES}\n${CONFIRM_POPOVER_STYLES}\n${SUBSCRIBE_PLANS_STYLES}\n${ONBOARDING_DESIGN_STYLES}\n${READLIST_DESIGN_STYLES}`,
		bodyClass: READLIST_DESIGN_BODY_CLASS,
		pinnedAppearance: "light",
		content: { html: content },
		scripts: scripts.join("\n"),
	};
}

const autoSubmitScript = (cspNonce: CspNonce) => `
<script nonce="${cspNonce}">
	(function () {
		function run() {
			var form = document.querySelector('[data-auto-submit]');
			if (form) form.requestSubmit();
		}
		if (document.readyState === 'loading') {
			document.addEventListener('DOMContentLoaded', run, { once: true });
		} else {
			run();
		}
	})();
</script>
`;
