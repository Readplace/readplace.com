...
const DEEP_FILTERS: ReadlistUrlState = { readlist: DEFAULT_READLIST_SLUG, tab: "done", order: "asc", page: 2 };
...
describe("renderReadlistCountsTrigger", () => {
	it("renders an inert loader span on the page (no OOB attribute)", () => {
		const span = parse(
			renderReadlistCountsTrigger({ countsUrl: "/queue/counts?tab=done" }),
		).getElementById("readlist-counts");
		...
		assert.equal(span.hasAttribute("data-test-readlist-counts"), true);
	});

	it("re-arms an identical span carrying hx-swap-oob for the mutation response", () => {
		const span = parse(
			renderReadlistCountsTrigger({ countsUrl: "/queue/counts", oob: true }),
		).getElementById("readlist-counts");
...
describe("renderReadlistMutationFragment", () => {
	it("wraps the toast in the stable #status-toast mount, re-arms counts, and leaves the primary body empty so the card is removed", () => {
		...
		assert.equal(doc.querySelector(".readlist-article"), null, "no card markup in the primary body");
	});
