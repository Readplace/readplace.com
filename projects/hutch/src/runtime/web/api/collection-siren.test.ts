...
describe("toArticleCollectionEntity", () => {
	...
	it("advertises a title-less web-app door on the iOS surface so only a build that recognises the rel opens the web queue in its own sheet", () => {
		const result: FindArticlesResult = {
			articles: [],
			total: 0,
			hasMore: false,
		...
		expect(entity.links).toContainEqual({
			rel: ["web-app"],
			href: "/queue?platform=ios",
		});
	});

	it("points the Android surface's door at its own platform marker", () => {
		const result: FindArticlesResult = {
			articles: [],
			total: 0,
			hasMore: false,
		...
		const entity = toArticleCollectionEntity(result, {}, { surfacePlatform: "android" });

		expect(entity.links).toContainEqual({
			rel: ["web-app"],
			href: "/queue?platform=android",
		});
	});

	it("omits the web-app door for a browser or extension Siren client, which has no app sheet to host it", () => {
		const result: FindArticlesResult = {
			articles: [],
			total: 0,
			hasMore: false,
		...
		expect(entity.links?.some((l) => l.rel.includes("web-app"))).toBe(false);
	});

	it("leaves the door title-less so an already-shipped build that does not recognise the rel renders no control", () => {
		const result: FindArticlesResult = {
			articles: [],
			total: 0,
			hasMore: false,
		...
		const door = entity.links?.find((l) => l.rel.includes("web-app"));
		assert(door, "the iOS surface must carry a web-app door");
		expect(door.title).toBeUndefined();
	});

	it("keeps the door on the queue root rather than the collection's current filter", () => {
		const result: FindArticlesResult = {
		...
		const entity = toArticleCollectionEntity(
			result,
			{ status: "read", order: "asc", page: 3 },
			{ surfacePlatform: "ios" },
		);

		const door = entity.links?.find((l) => l.rel.includes("web-app"));
		assert(door, "the iOS surface must carry a web-app door");
		expect(door.href).toBe("/queue?platform=ios");
	});

	it("emits the door beside the account link", () => {
		const result: FindArticlesResult = {
			articles: [makeArticle("1")],
			total: 1,
			hasMore: false,
		...
		expect(linkRels).toEqual(["self", "root", "account", "web-app", "add-links-help"]);
	});
