...
function createSqsEvent(
	...
	return {
		Records: [{
			...
			md5OfBody: "",
			eventSource: "aws:sqs",
			eventSourceARN: "arn:aws:sqs:ap-southeast-2:123456789:save-link-raw-pdf-command-dlq",
			awsRegion: "ap-southeast-2",
		}],
	};
}

describe("initSaveLinkRawPdfDlqHandler", () => {
	it("dispatches the markCrawlExhausted transition with the URL, reason, and receiveCount from the DLQ record", async () => {
		...
		const handler = initSaveLinkRawPdfDlqHandler({
