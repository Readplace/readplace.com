...
function crawlFailedArticle(url: string): Article {
	return {
		...pendingArticle(url),
...
function crawlUnsupportedArticle(url: string): Article {
	return {
		...pendingArticle(url),
...
describe("initGenerateSummaryHandler", () => {
	...
	it("marks the summary skipped (reason='crawl-failed') when content is absent and the crawl terminally failed, instead of DLQing forever", async () => {
		const URL = "https://example.com/crawl-failed";
		const { handler, deps } = createHandler({
			findArticleContent: jest.fn<ReturnType<FindArticleContent>, Parameters<FindArticleContent>>().mockResolvedValue(undefined),
			loadArticle: jest.fn().mockResolvedValue(crawlFailedArticle(URL)),
		});
	...
	it("marks the summary skipped (reason='crawl-unsupported') when content is absent and the crawl is unsupported", async () => {
		const URL = "https://example.com/crawl-unsupported";
		const { handler, deps } = createHandler({
			findArticleContent: jest.fn<ReturnType<FindArticleContent>, Parameters<FindArticleContent>>().mockResolvedValue(undefined),
			loadArticle: jest.fn().mockResolvedValue(crawlUnsupportedArticle(URL)),
		});
	...
	it("keeps the assert → DLQ path when content is absent and no aggregate row exists (unknown row, not a terminal crawl)", async () => {
		const URL = "https://example.com/no-row";
		const { handler, deps } = createHandler({
			findArticleContent: jest.fn<ReturnType<FindArticleContent>, Parameters<FindArticleContent>>().mockResolvedValue(undefined),
