 ...
 * its `(N)` counts in lockstep with the panel — exactly like the header badge.
 * The full page renders it inline (`oob: false`); the poll routes re-emit it
 * with `oob: true` so an htmx out-of-band swap fills the counts in the instant
 * extraction completes, instead of leaving them withheld until a full reload.
 */
