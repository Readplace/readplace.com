 ...
 *
 * The lookup only fires when there is something to resolve: an authenticated
 * request whose `emailVerified` standing is not yet `true`. The cookie session
 * stores that standing at login, so guests, known-verified sessions, and a
 * request already resolved upstream short-circuit with no read; only a session
 * minted for an unverified (or legacy) account hits the record.
 *
