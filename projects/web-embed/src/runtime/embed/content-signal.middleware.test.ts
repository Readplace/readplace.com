...
describe("contentSignalMiddleware", () => {
	it("sets the site-wide Content-Signal policy on GET page responses", () => {
		const { headers, res } = createFakeRes();
		const next = jest.fn() as unknown as NextFunction;

		contentSignalMiddleware({ method: "GET", path: "/" } as Request, res, next);

		expect(headers["Content-Signal"]).toBe("search=yes, ai-input=yes, ai-train=no");
	...
	it("varies on Accept so a CDN keys HTML and markdown separately", () => {
		...
		contentSignalMiddleware({ method: "GET", path: "/preview" } as Request, res, next);
