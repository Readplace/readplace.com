...
describe("rendered canonical snippet invariants", () => {
	...
	const expectedCanonicalHtml: Record<SnippetVariant, string> = {
		...
		b: '<a href="https://readplace.com/save?url=PAGE_URL&amp;save_surface=embed" style="display:inline-flex;align-items:center;gap:8px;padding:8px 14px;background:#2B3A55;color:#FFFFFF;text-decoration:none;font:600 14px/1 Inter,-apple-system,system-ui,sans-serif;border-radius:6px;border:1px solid #2B3A55">\n  <img src="https://readplace.com/embed/icon.svg?v=2" alt="" width="20" height="20" style="display:block;border:0">Save to Readplace\n</a>\n',
...
describe("renderSnippet", () => {
	it("should substitute appOrigin in the save link href", () => {
		...
		expect(html).toContain('href="http://127.0.0.1:9999/save?url=PAGE_URL&amp;save_surface=embed"');
	});
	...
	it("should substitute pageUrl in the save link href", () => {
		...
		expect(html).toContain('href="https://readplace.com/save?url=https://example.com/my-article&amp;save_surface=embed"');
	});
