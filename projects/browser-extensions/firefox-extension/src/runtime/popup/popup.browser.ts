...
// what they mean. Every rendering decision is made upstream; this glue only
// paints it. The message html is the server-authored text/html body, injected
// as HTML — trusted by contract.
...
async function saveAllTabsFlow() {
	...
	const { saveable, skipReasons } = classifyTabs(tabs, __APP_DOMAINS__);
	...
	if (failedListEl) {
		const lines = buildSaveAllDetailLines({ ...result.value, clientSkipReasons: skipReasons });
