...
export const SAVED_TINT = [61, 139, 110] as const;
...
export function compositeSavedIcon(source: { glyph: Rgba; haloed: Rgba }): Rgba {
	const { glyph, haloed } = source;
	...
	for (let i = 0; i < data.length; i += 4) {
		...
		for (let channel = 0; channel < 3; channel++) {
			const blended =
				SAVED_TINT[channel] * tintAlpha + haloed.data[i + channel] * haloAlpha;
