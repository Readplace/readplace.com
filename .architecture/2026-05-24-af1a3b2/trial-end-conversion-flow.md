...
- **Auth trial-signup (email + Google OAuth)** — both signup paths now call `createTrialEndSchedule({ userId, firesAt: trialEndsAt })` immediately after `upsertTrialing`. Schedule creation failure fails the signup with 500 (consistent with Phase 1's "fail loudly" stance).
...
|---|---|---|
| Trial-end schedule creation fails at signup | 500 to user; trial row may persist as orphan | User retries signup (idempotent via email-uniqueness); operator can manually create the schedule from the existing row |
