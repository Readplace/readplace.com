...
A DLQ carries two alarms with two owners. The component that owns the queue raises the **backlog** alarm on how many dead letters are waiting — the queue-backed Lambda component for a queue that mints its own DLQ, the shared-DLQ component for one that takes a shared DLQ. Wherever a DLQ also has a consumer, that single consumer raises an **arrival** alarm on how many dead letters it received, pointing at the SNS topic the owner already created: the consumer drains the queue within its polling interval, often between two samples of the depth gauge, so a dead letter that is picked up and terminated leaves no backlog for the owner's alarm to measure and would otherwise page no one. A shared DLQ needs one consumer that routes each dead letter back to the failure handler for the queue it came from — the dead-letter router, the workspace package whose single runtime export routes each dead letter to the failure handler registered for its source queue (its unknown-queue error reads `No dead-letter route registered`).
