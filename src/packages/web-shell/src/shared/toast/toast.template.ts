export const TOAST_TEMPLATE = `<div class="toast" tabindex="-1" data-dismiss="{{dismissMs}}" data-test-toast>
	...
	<form class="toast__action-form" method="{{method}}" action="{{url}}" hx-boost="true" hx-target="main" hx-select="main" hx-swap="outerHTML show:none" hx-disabled-elt="find button">
		...
		<button class="btn btn--primary btn--compact toast__action" type="submit" aria-label="{{label}}" data-test-toast-action><span class="toast__action-label">{{label}}</span><span class="toast__action-loader" aria-hidden="true"><span></span><span></span><span></span></span></button>
	</form>
	{{/each}}
</div>`;
