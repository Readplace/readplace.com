export const CONFIRM_POPOVER_TEMPLATE = `<div class="confirm-popover{{#if wide}} confirm-popover--wide{{/if}}{{#if illustrationHtml}} confirm-popover--illustrated{{/if}}" id="{{id}}" popover="auto" role="dialog" tabindex="-1" autofocus aria-labelledby="{{id}}-title" aria-describedby="{{describedBy}}" data-test-confirm-popover="{{key}}"{{#if subject}} data-test-confirm-subject="{{subject}}"{{/if}}{{#if openBeaconUrl}} data-beacon-url="{{openBeaconUrl}}"{{/if}}>{{#if illustrationHtml}}<div class="confirm-popover__illustration" aria-hidden="true">{{{illustrationHtml}}}</div>{{/if}}
