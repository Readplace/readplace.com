...
export function brandMarkSvg(opts: { className?: string } = {}): string {
	const decorative = opts.className
		...
		: "";
	return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"${decorative}><rect x="15" y="15" width="482" height="482" rx="102" fill="#2B3A55" stroke="#FFFFFF" stroke-opacity="0.4" stroke-width="20"/><path d="${AMPERSAND_PATH}" fill="#FFFFFF"/><circle cx="353" cy="182" r="44" fill="#C8923C"/></svg>`;
}
