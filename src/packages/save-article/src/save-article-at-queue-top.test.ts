...
import { initSaveArticleAtQueueTop } from "./save-article-at-queue-top";
...
describe("initSaveArticleAtQueueTop", () => {
	it("allocates one queue position for the caller's user and stamps the save with it", async () => {
		const calls: string[] = [];
	...
	it("never reaches the save when the position allocation fails", async () => {
		...
		await expect(
			saveArticleAtQueueTop({ userId, url: exampleUrl, freshness: { action: "skip" }, provenance }),
