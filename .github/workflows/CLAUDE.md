...
| `publish-firefox-extension.yml` | Publish the Firefox extension to AMO as a listed (public) add-on, with the listing metadata its `sign` script passes via `--amo-metadata` | Called by `ci.yml` |
| `sync-signed-extension.yml` | Mirror the newest AMO-approved Firefox XPI to S3 for installs that still poll the self-hosted update manifest and for `/install?client=firefox` | Schedule (every 12h) / manual |
...
| `publish-ios-testflight-notes.yml` | Set an already-uploaded build's TestFlight "What to Test" note from the App Store release notes via `whats_new`; `status` reads release states without changes, and `external` explicitly submits the build for public beta review using existing public groups and review-account secrets. Uploads no binary and does not email testers | Manual |
