...
4. **Confirm the outcome on the production row**, after its `crawlStatus` is `ready` again. A promotion logs `[RecrawlContentExtracted] promoted tier to canonical` and moves `summaryStatus` to `pending`, then `ready` or `failed`. `[RecrawlContentExtracted] tie kept canonical unchanged` means the tier sources tied and the canonical stayed, so the summary did not re-run and another recrawl will not change that: recovering that row needs a change to the recrawl's tie path, with the row as evidence.
